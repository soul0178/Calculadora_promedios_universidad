...
function onAction(e){
  ...
  if(act==='toggle-theme'){ toggleTheme(); render(); }
  else if(act==='go-dashboard'){ state.view={tab:'dashboard',courseId:null, showArchived:state.view.showArchived}; render(); }
  ...
  else if(act==='open-add-past-semester'){ openAddPastSemesterModal(); }
  else if(act==='reopen-semester'){ reopenSemester(id); }
  else if(act==='export-course-pdf'){ exportCoursePDF(id); }
  else if(act==='export-semester-pdf'){ exportSemesterPDF(id); }
  else if(act==='cloud-sign-in'){ signInWithGoogle(); }
  else if(act==='cloud-sign-out'){
    if(confirm('¿Cerrar sesión? Tus datos siguen guardados en este dispositivo y en la nube, pero dejan de sincronizarse hasta que vuelvas a iniciar sesión.')) signOutCloud();
  }
}
