/* ============================= SEMESTRES ============================= */
// Cursos activos (no archivados) a los que aún les falta ingresar alguna nota.
// Los cursos archivados quedan exentos: archivar es la forma de decir "este
// curso ya no cuenta para el cierre normal" (p.ej. un curso retirado).
function incompleteCourses(){
  return state.courses.filter(c => !c.archived && missingIndices(c).length>0);
}
function closeSemester(label){
  ...
  if(incomplete.length>0){
    alert('No puedes terminar el semestre: todavía hay cursos con notas pendientes:\n\n'
      + incomplete.map(c=>'• '+(c.name||'(sin nombre)')).join('\n'));
    return false;
  }
