/* ============================= RENDER ============================= */
function getCareerClass(name){
  if(!name) return '';
  const lower = name.toLowerCase();
  if(lower.includes('computación') || lower.includes('computacion')) return 'cc';
...
function makeLineChart(canvasId, seriesActual, seriesProj, extraOpts){
  ...
  if(!el) return;
  const cRed=cssVar('--red'), cAmber=cssVar('--amber'), cInkDim=cssVar('--ink-dim'), cGrid=cssVar('--chart-grid');
  chartInstances[canvasId] = new Chart(el, {
    type:'line',
    data:{ labels:SHORT, datasets:[
      {label:'Real', data:seriesActual, borderColor:cRed, backgroundColor:cRed, pointRadius:4, tension:.25, spanGaps:false},
      {label:'Proyección', data:seriesProj, borderColor:cAmber, borderDash:[6,4], pointRadius:2.5, tension:.25, spanGaps:true}
    ]},
    options:{ responsive:true, scales:{ y:{min:0,max:20, grid:{color:cGrid}, ticks:{color:cInkDim}}, x:{grid:{display:false}, ticks:{color:cInkDim}}}, plugins:{legend:{labels:{color:cInkDim}}} }
  });
}
...
function renderCloudAuthSection(){
  // Si js/cloud-sync.js no está cargado, o no configuraste tu proyecto de
  // Firebase en js/firebase-config.js, no se muestra nada (la app funciona
  // 100% local, como siempre).
  if(typeof cloudConfigured !== 'function' || !cloudConfigured()) return '';
  if(typeof currentUser !== 'undefined' && currentUser){
    return `<span class="sync-hint" title="Sincronizado con tu cuenta de Google">☁️ ${esc(currentUser.displayName || currentUser.email || 'Sesión iniciada')}</span>
      <button class="btn btn-ghost btn-sm" data-act="cloud-sign-out">Cerrar sesión</button>`;
  }
  return `<button class="btn btn-ghost btn-sm" data-act="cloud-sign-in">🔑 Iniciar sesión con Google</button>`;
}
...
function renderTopbar(){
  ...
  <div class="topbar">
    <div class="brand">
      <img src="icons/icon-192.png" alt="Logo" style="width: 36px; height: 36px; border-radius: 50%; object-fit: cover;">
      <div><h1>Libreta</h1><div class="sub">calculadora y proyección de notas</div></div>
    </div>
    <div class="toolbar">
      ${renderCloudAuthSection()}
      <button class="btn btn-ghost btn-sm" data-act="toggle-theme" title="Cambiar a tema ${getTheme()==='dark'?'claro':'oscuro'}">${getTheme()==='dark'?'🌙':'☀️'}</button>
...
function renderDashboard(){
  ...
  // 2) Semestre actual
  const pendingCount = incompleteCourses().length;
  html += `<div class="section-title"><h2>Semestre actual</h2><div class="line"></div>
    ${pendingCount>0?`<span class="badge badge-warn" title="Estos cursos deben completar sus notas antes de poder cerrar el semestre">⚠ ${pendingCount} curso(s) con notas pendientes</span>`:''}
    ...
    let rows = active.map(c=>{
      ...
      return `<div class="course-row" data-act="open-course" data-id="${c.id}">
        <div>
          ...
          <div class="meta-tag"><span class="career-chip ${getCareerClass(careerName(c.careerId))}">${esc(careerName(c.careerId))}</span><span>${c.creditos>0? c.creditos+' cr.' : 'sin créditos'}</span>${c.archived?'<span>· archivado</span>':''}</div>
        </div>
  ...
  html += `<div class="section-title"><h2>Historial de semestres</h2><div class="line"></div>
    <button class="btn btn-ghost btn-sm" data-act="open-add-past-semester">+ Agregar semestre pasado</button>
  </div>`;
      ...
      html += `<div class="semester-card">
        <div class="sem-head">
          ...
          <div class="toolbar">
            ...
            <button class="btn btn-ghost btn-sm" data-act="export-semester-pdf" data-id="${sem.id}">📄 Exportar PDF</button>
...
function renderCourseDetail(id){
  ...
  <div class="detail-header">
    <div>
      ...
      <div class="meta-tag" style="font-size:13px;"><span class="career-chip ${getCareerClass(careerName(c.careerId))}">${esc(careerName(c.careerId))}</span><span style="color:var(--ink-faint);">${c.creditos>0? c.creditos+' créditos':'sin créditos asignados'}</span></div>
    </div>
    <div class="toolbar">
      <button class="btn btn-ghost btn-sm" data-act="export-course-pdf" data-id="${c.id}">📄 Exportar PDF</button>
